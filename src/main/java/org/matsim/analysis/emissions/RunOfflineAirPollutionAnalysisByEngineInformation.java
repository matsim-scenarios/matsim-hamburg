	...
	private void prepareNetwork(HamburgExperimentalConfigGroup hamburgCfg, Scenario scenario) {
		...
		for (Link link : scenario.getNetwork().getLinks().values()) {
			...
			if(freespeed <= 8.333333333){ //30kmh
				link.getAttributes().putAttribute(EmissionUtils.HBEFA_ROAD_TYPE, "URB/Access/30");
			} else if(freespeed <= 11.111111111){ //40kmh
				link.getAttributes().putAttribute(EmissionUtils.HBEFA_ROAD_TYPE, "URB/Access/40");
				...
				if(lanes <= 1.0){
					...
					link.getAttributes().putAttribute(EmissionUtils.HBEFA_ROAD_TYPE, "URB/Distr/50");
				} else if(lanes > 2.0){
					link.getAttributes().putAttribute(EmissionUtils.HBEFA_ROAD_TYPE, "URB/Trunk-City/50");
				} else{
				...
				if(lanes <= 1.0){
					...
					link.getAttributes().putAttribute(EmissionUtils.HBEFA_ROAD_TYPE, "URB/Trunk-City/60");
				} else if(lanes > 2.0){
					link.getAttributes().putAttribute(EmissionUtils.HBEFA_ROAD_TYPE, "URB/MW-City/60");
				} else{
					throw new RuntimeException("NoOfLanes not properly defined");
				}
			} else if(freespeed <= 19.444444444){ //70kmh
				link.getAttributes().putAttribute(EmissionUtils.HBEFA_ROAD_TYPE, "URB/MW-City/70");
			} else if(freespeed <= 22.222222222){ //80kmh
				link.getAttributes().putAttribute(EmissionUtils.HBEFA_ROAD_TYPE, "URB/MW-Nat./80");
			} else if(freespeed > 22.222222222){ //faster
				link.getAttributes().putAttribute(EmissionUtils.HBEFA_ROAD_TYPE, "RUR/MW/>130");
			} else{
